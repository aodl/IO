...
The committed package includes a renderable local `sns_init` candidate, per-run runtime inputs, evidence capture helpers, no-network validators, and restartable phases 12–17. Those phases verify exact IO release provenance, install Paused dapps, publish a reviewed Governance/Root bundle through SNS-W, finalize and discover the SNS, submit real treasury and lifecycle proposals, exercise production redemption, and capture index/archive/controller evidence. They fail closed and persist checkpoints; implemented automation is not a claim that a fresh run passed. The prior one-component candidate result remains historical evidence of a Governance/official-Root `unit_variant` incompatibility, and the maintained chunk-store route remains an upstream tooling risk until the same-source run establishes its disposition.
