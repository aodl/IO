...
The committed package includes a renderable local `sns_init` candidate, per-run runtime inputs, evidence capture helpers, no-network validators, and restartable phases 12–17. Those phases verify exact IO release provenance, install Paused dapps, publish a reviewed Governance/Root bundle through executed local NNS Governance proposals into SNS-W, verify its exact compressed hashes, finalize and discover the SNS, submit real treasury and lifecycle proposals, exercise production redemption, and capture index/archive/controller evidence. If the maintained chunk-store CLI route fails before execution, phase 17 submits the exact release Wasm inline through a signed SNS Governance proposal and Root; it records proposal and before/after module hashes and does not claim a module change for a same-Wasm result. The phases fail closed and persist checkpoints; implemented automation is not a claim that a fresh run passed. The prior one-component candidate result remains historical evidence of a Governance/official-Root `unit_variant` incompatibility until the same-source run establishes its disposition.
