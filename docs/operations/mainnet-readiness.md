...
- completed official local SNS ledger rehearsal against real SNS-created local ledger/index/governance/root canisters;
- final validated official `sns_init.yaml`;
- completed optional local SNS rehearsal following current official ICP/DFINITY SNS testing documentation;
...
The official local SNS rehearsal is complete only after official local SNS tooling and SNS-W have run locally, local SNS root/governance/ledger/index/swap IDs are recorded, real local SNS ledger fee disposition/total-supply delta/protocol reserve balance observations are captured, reserve-to-user, user-to-redemption, and redemption-to-reserve transfers are observed separately, bad-fee/insufficient-funds/duplicate behavior and duplicate block proof are recorded, index account history is observed, SNS governance/root/swap availability and dapp controller state are checked, and `cargo run -p xtask -- validate_local_sns_ledger` passes against the filled local evidence file. Even then, the result does not prove mainnet SNS launch readiness, final tokenomics, final SNS config, mainnet testflight, audit readiness, or production adapter activation.
...
We do not currently run the official SNS launch locally in required CI. Official SNS testing is optional and heavier. The current official ICP/DFINITY SNS testing documentation is the source of truth; the historical standalone `dfinity/sns-testing` repository is deprecated. The maintained official local SNS flow uses the source-built `sns` CLI; this is optional/manual and local-only for the local rehearsal layer, not part of required IO workflows. SNS testflight is a future manual/mainnet rehearsal. IO's canonical IO ledger should be the SNS ledger, and it is not launched on mainnet. Any IO_TEST ledger is non-canonical. The existing canister that owns IO NNS neuron 6345890886899317159 is not touched by these tests.

Production scan/index adapters, live governance adapters, historian production ingestion adapters, archive traversal, fee policy, index lag alerting, official local SNS ledger rehearsal, SNS launch configuration, production SNS root/governance wiring, fallback-controller handoff, and duplicate-transfer proof checks against real blocks must be finalized before any mainnet deployment proposal.

## 2026-07-28 Truth-Pass Fee/Supply Update

Local SNS rehearsal evidence must record observed fee disposition and total-supply deltas. The current standard SNS ledger path observed at `dfinity/ic@2d7f90fb23672cc3b81c216a33d04c75672dd308` does not set a fee collector, and the ICRC ledger burns transfer fees when no collector is configured. Readiness claims must not state that reserve-transfer issuance/redemption always preserves total supply. IO remains not live and the reserved production canisters remain inert.
