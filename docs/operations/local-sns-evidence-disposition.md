...
Current launch readiness therefore requires a later immutable package marked `canonical_redemption_economics = true`. That package must preserve the exact Stream, NNS-manager, historian, SNS-init and Account-map inputs; prove the derived treasury Account and nonzero observed balance; and pass checked cross-consistency for the Stream quote, both ledgers and the historian snapshot. Historical package integrity remains independently verifiable and is not described as corruption.
