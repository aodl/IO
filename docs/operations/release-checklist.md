...
- [ ] Before integration, prove the pull-request head is the checked-out `HEAD` and the current PR base commit is an ancestor of it. Do not merge the base into the selected release tail or rebase the branch to repair a failed ancestry check, except through the pre-source graph-only reconciliation defined below.
...
- [ ] One graph-only ancestry reconciliation may precede source finalization only when machine checks prove the reviewed base is absent solely by graph shape, its complete tree equals an existing release ancestor, their diff is empty, and the merge tree equals its first-parent tree. The merge is an integration/provenance operation, grants no authority to change release contents, and must not appear inside the selected release tail. Establish a new source-finalization commit, immediate artifact-recording child, and fresh canonical evidence package after it. Content-bearing bases require normal source review; do not squash, rebase, or replay solely to repair a tree-identical divergence.
