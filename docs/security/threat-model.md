...
| Governance boundary confusion | NNS/SNS governance records and traits are production-shaped, while mock adapters are clearly scoped to debug canisters. | Governance Candid fixture tests, mock adapter type names, DID guardrails. | Real NNS/SNS adapters are not audited or wired. | Audit Candid mappings, pagination, retry policy, and proposal/neuron reconciliation before mainnet. |
| Reward allocation includes excluded SNS neurons | Eligibility snapshots carry exclusion reasons for Jupiter governance, protocol-owned, dissolving, short-delay, and zero-stake neurons. | SNS eligibility and allocation tests. | Real SNS snapshots and launch config remain future work. | Reconcile final SNS genesis neurons and protocol-owned IDs before enabling rewards. |
