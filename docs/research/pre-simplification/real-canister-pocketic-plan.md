...
1. Extend stream-manager value-moving real ledger/index scanner processing beyond the proven Jupiter deposit/issuance, exact backed two-week reward distribution, strict four-role participation reward reconciliation, redemption payout/return, fee/dust accounting, below-return-fee closed failure, over-redeemable-supply rejection, index-lag no-fabrication/catch-up, holder-yield current-rate redemption, staker-reward rate-preservation, and completed-redemption same-Wasm upgrade paths to malformed/duplicate redemption variants and partial-operation retry/upgrade cases against the local ICP index and finalized SNS index already wired into the fixture.
2. Add failure-injection upgrade/retry/idempotency tests around partially completed finalized-governance active-stake refresh and future real scanner operations.
