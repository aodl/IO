Status: historical research; non-normative; not launch architecture.
...
20. `io_real_stack_installs_stream_manager_on_application_subnet`, `io_real_stack_installs_nns_neuron_manager_on_application_subnet`, and `io_real_stack_installs_historian_on_application_subnet` reuse the finalized SNS fixture and install `io_stream_manager`, `io_nns_neuron_manager`, and `io_historian` Wasms on the application subnet with local NNS ledger/governance and finalized SNS ledger/index/governance IDs. This historical plan described companion checks for the then-active development-public-shell model; it is non-normative and is not part of current production wiring.
21. `io_stream_manager_real_finalized_sns_list_neurons_updates_active_staked_io` uses locally built debug IO Wasms for evidence, calls stream-manager `debug_tick`, consumes finalized SNS governance `list_neurons`, and verifies `active_staked_io_e8s` equals finalized non-dissolving exact two-week SNS neuron stake.
...
27. `io_stream_manager_real_redemption_rounding_fee_dust_accounted`, `io_stream_manager_real_redemption_below_io_return_fee_fails_closed`, `io_stream_manager_real_redemption_rejects_insufficient_redeemable_supply`, and `io_stream_manager_real_redemption_after_index_lag_waits_or_fails_closed` cover fee journal evidence, sub-return-fee closed failure before ICP payout, over-redeemable-supply rejection without payout/accounting mutation/fabricated success evidence, and index-lag no-fabrication/catch-up behavior.
28. `io_stream_manager_real_redemption_after_holder_yield_is_higher_than_genesis` scans a Jupiter deposit and a two-year maturity event through the local ICP ledger/index in one stream-manager tick, verifies descending account-history cursor handling against the real local ICP index, then redeems IO at the increased 1.1 ICP/IO current rate through the finalized SNS ledger/index and local NNS ledger payout path.
29. `io_stream_manager_real_redemption_after_staker_rewards_preserves_rate` scans a Jupiter deposit and 5 ICP two-week reward distribution through the local ICP ledger/index in one stream-manager tick, transfers the exact backed 3 IO reward pool through the finalized SNS ledger, verifies liquid backing and redeemable supply both become 63 IO-equivalent e8s, and redeems 10 IO at the unchanged 1.0 ICP/IO current rate.
30. `real_sns_root_control_uses_application_subnet_canister_direct_root_path` direct-installs real SNS root with a local application-subnet dapp and verifies `list_sns_canisters`.
...
- extend stream-manager reward/broader current-rate redemption and historian real-stack checks against the finalized local SNS canister IDs, and add browser E2E on top of the existing frontend source-backed static honesty checks.
