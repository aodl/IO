...
The stream manager persists scan progress:
...
The legacy highest-block fields remain compatibility checkpoints. The account-history scan fields carry page order, `latest_cursor`, `oldest_cursor`, backfill-complete, and internal health/status counters. ICP account history is treated as descending/newest-first when detected; descending pages are applied in chronological order and maintain separate head catch-up and historical backfill progress. Ascending mock/IO scans retain forward cursor semantics and tolerate one repeated cursor row.

Scans use index canisters as the normal account-history abstraction; raw ledger/archive traversal is not wired into scheduler execution. Account-history global block gaps are normal. A cursor advances only after a relevant observed block is safely represented in the journal, represented as a terminal rejected operation, or is known to be already processed, and downstream transfer progress remains journal-gated before a scan page is committed. The processed-transaction set remains a duplicate guard for replayed source transaction IDs.
...
Client modules now exist for ICP ledger/index, IO ledger/index, SNS governance, NNS governance, and ledger transfer calls. The shared `io-ledger-types` crate defines the production-shaped ledger/index boundary used by real adapter structs and mock mapping code, including account-history scan order/progress modelling. The shared `io-governance-types` crate defines NNS/SNS governance boundaries, production-shaped NNS/SNS Candid DTOs, Wasm-gated governance canister client structs, SNS eligibility snapshots, and SNS participation summaries. Downstream transfer paths now use `LedgerTransferClient`; debug/PocketIC scan sources still use mock/local index and transaction-history APIs. Production-shaped ICP/ICRC ledger and index adapters are fixture-tested but not audited or wired into default production execution. Production-shaped NNS/SNS governance adapters are also fixture-tested only and not wired into default production execution. Full archive traversal, audited retry policy, final governance principal wiring, certified frontend data, and production lifecycle reconciliation remain future work. `io_historian` now provides the public read-model surface for bounded dashboard state, but its production source adapters remain future-wired; local/test ingestion feeds observations without calling value-moving private state. Boundary tests cover transfer errors, duplicate transfer proof, fees, account identifiers, account-history cursor gaps, full-ledger-contiguous cursor gaps, archive-required pages, index lag, descending/newest-first pages, production-shaped index page mapping, governance Candid fixtures, governance error classification, governance pagination, SNS eligibility, and SNS participation. Production wiring remains future work and should preserve ledger/index/governance/timer-driven flows rather than caller-submitted stream kinds. The journal is production-shaped but not audited.
...
## 2026-07-28 Truth Pass

The scheduler remains inactive for production value movement. Before timers or production adapters are enabled, the redemption path must preserve exact user destinations, use immutable created-at/memo/fee attempts, reconcile IO-return and ICP-payout fees against canonical ledger evidence, and remove release-reachable mock fallbacks. Reward eligibility, allocation, cohort weighting, participation, forfeiture, and dust semantics are unchanged by this readiness pass.
