...
The local SNS harness adds documentation, fixture skeletons, deterministic xtask guardrails, and a PocketIC topology smoke test. The smoke test installs IO canisters with SNS-shaped local principals and checks that production value-moving DIDs remain constructor-only. It does not run official SNS launch tooling and does not wire value-moving flows to local SNS ledger/index canisters.
