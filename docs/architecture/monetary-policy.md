...
Economic meaning follows the protocol-controlled Account holding fungible ICP. IO does not track that ICP's upstream provenance after custody. The fixed two-week and two-year maturity staging subaccounts are domain-separated and owned by the NNS Manager. A balance delta above the persisted pre-command baseline freezes the captured amount; donations arriving before capture intentionally inherit that Account's semantics.
