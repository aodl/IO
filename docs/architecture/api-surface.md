...
backing first obtains a no-effect NNS readiness result, freezes at most one
...
`observe_two_week_backing_readiness`, `prepare_two_week_maturity`,
...
target. The readiness method is authenticated and read-only; the generic
