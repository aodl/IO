...
Stable-storage validation treats this document as the retention boundary: compaction applies only to historian/read-model history, never to an active monetary intent, caller replay result, pending entitlement batch, pending maturity or pending unwind.
