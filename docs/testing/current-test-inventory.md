...
- `tests/pocketic/io_stream_manager_pocketic.rs`: Paused install, lifecycle validator payload/rendering, same-Wasm upgrade and narrow ingress safety.
- `tests/pocketic/io_nns_neuron_manager_pocketic.rs`: Paused install, lifecycle validator payload/rendering, same-Wasm upgrade and authority rejection.
...
- `tools/scripts/provision-icp-cli`: immutable official `icp` 0.2.7 HTTPS provisioning with archive and executable SHA-256 verification, used by ordinary PR CI before required `icp project show` and `icp build` checks.
